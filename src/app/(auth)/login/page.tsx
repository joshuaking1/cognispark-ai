  ...
  const handleLogin = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    if (error) {
      ...
      const { data: profile, error: profileError } = await supabase
        .from("profiles")
        .select("role, has_completed_onboarding") // Also fetch onboarding status
        .eq("id", loginData.user.id)
        .single();

      if (profileError && profileError.code !== "PGRST116") {
        // Allow profile not existing yet if trigger is slow
        toast.error("Login Success, but Profile Error", {
          description:
            "Could not determine your role. Please try again or contact support.",
        });
        // Fallback redirect
        ...
        if (profile?.role === "teacher" || profile?.role === "admin") {
          router.push("/teacher-portal"); // Redirect teachers/admins to teacher portal
        } else {
          router.push("/dashboard"); // Redirect students to student dashboard
        }
        router.refresh();
      }
    }
